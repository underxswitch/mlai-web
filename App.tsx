...
const AuthorCard = ({ name, role, delay }: { name: string, role: string, delay: string }) => {
  return (
    <div className="flex flex-col group animate-fade-in-up items-center p-10 bg-white rounded-2xl border border-stone-200 shadow-sm hover:shadow-xl transition-all duration-500 w-full max-w-xs hover:-translate-y-2 hover:border-nobel-gold/40 relative overflow-hidden" style={{ animationDelay: delay }}>
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-nobel-gold/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
      <div className="w-16 h-16 bg-stone-50 rounded-full flex items-center justify-center mb-6 border border-stone-100 group-hover:border-nobel-gold/20 transition-colors duration-500">
        <span className="font-serif text-2xl text-stone-300 group-hover:text-nobel-gold/50 transition-colors duration-500">{name.charAt(0)}</span>
      </div>
      <h3 className="font-serif text-2xl text-stone-900 text-center mb-3 group-hover:text-nobel-gold transition-colors duration-500">{name}</h3>
      <div className="w-12 h-0.5 bg-nobel-gold mb-4 opacity-40 group-hover:opacity-100 group-hover:w-20 transition-all duration-500"></div>
      <p className="text-[10px] text-stone-400 font-bold uppercase tracking-[0.2em] text-center leading-relaxed group-hover:text-stone-600 transition-colors duration-500">{role}</p>
    </div>
  );
};

const PartnerLogo = ({ name, subtitle }: { name: string, subtitle?: string }) => (
  <div className="flex flex-col items-center justify-center p-8 grayscale hover:grayscale-0 transition-all duration-500 opacity-60 hover:opacity-100">
    <div className="font-serif text-2xl font-bold tracking-tighter text-stone-800 mb-1">{name}</div>
    {subtitle && <div className="text-[10px] uppercase tracking-widest text-stone-400 font-bold">{subtitle}</div>}
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F4] text-stone-800 selection:bg-nobel-gold selection:text-white">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#F9F8F4]/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <div className="w-8 h-8 bg-stone-900 rounded-full flex items-center justify-center text-white font-serif font-bold text-xl shadow-sm pb-1">M</div>
            <span className={`font-serif font-bold text-lg tracking-wide transition-opacity ${scrolled ? 'opacity-100' : 'opacity-0 md:opacity-100'}`}>
              MLAI <span className="font-normal text-stone-500">QUANTUM</span>
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium tracking-wide text-stone-600">
            ...
            <a href="#specs" onClick={scrollToSection('specs')} className="hover:text-nobel-gold transition-colors cursor-pointer uppercase">Technical Specs</a>
      ...
      <header className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6 text-center">
          <motion.div 
            ...
            className="inline-block mb-4 px-3 py-1 border border-stone-800 text-stone-800 text-xs tracking-[0.2em] uppercase font-bold rounded-full backdrop-blur-sm bg-white/30"
          >
            MLAI CORPORATION • QUANTUM RESEARCH
          </motion.div>
          <motion.h1 
      ...
      <main>
        ...
        <section id="introduction" className="py-24 bg-white">
          <div className="container mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-12 gap-12 items-start">
            ...
            <div className="md:col-span-8 text-lg text-stone-600 leading-relaxed space-y-6">
              ...
              <p>
                MLAI CORPORATION, in partnership with Google DeepMind and Google Quantum AI, has developed this visualization to showcase the power of neural decoding in the quest for fault-tolerant quantum computing.
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="specs" className="py-24 bg-stone-100 border-t border-stone-200">
          <div className="container mx-auto px-6">
            <div className="text-center mb-16">
              <div className="inline-block mb-3 text-xs font-bold tracking-widest text-stone-500 uppercase">TECHNICAL SPECIFICATIONS</div>
              <h2 className="font-serif text-4xl md:text-5xl mb-4 text-stone-900">AlphaQubit Architecture</h2>
              <p className="text-stone-500 max-w-2xl mx-auto">A deep dive into the neural network that powers the next generation of quantum error correction.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="p-8 bg-white rounded-xl border border-stone-200 shadow-sm">
                <h3 className="font-serif text-2xl text-stone-900 mb-4">Recurrent Transformer</h3>
                <p className="text-sm text-stone-600 leading-relaxed">
                  Utilizes a temporal transformer architecture to process sequences of syndrome measurements, capturing long-range correlations in time and space.
                </p>
              </div>
              <div className="p-8 bg-white rounded-xl border border-stone-200 shadow-sm">
                <h3 className="font-serif text-2xl text-stone-900 mb-4">Analog Decoding</h3>
                <p className="text-sm text-stone-600 leading-relaxed">
                  Unlike binary decoders, AlphaQubit processes "soft" analog information from the readout, allowing it to weigh the probability of different error scenarios.
                </p>
              </div>
              <div className="p-8 bg-white rounded-xl border border-stone-200 shadow-sm">
                <h3 className="font-serif text-2xl text-stone-900 mb-4">Hardware Adaptive</h3>
                <p className="text-sm text-stone-600 leading-relaxed">
                  Learns the specific noise profile of the Sycamore processor, including cross-talk and leakage, which are difficult to model analytically.
                </p>
              </div>
            </div>
          </div>
        ...
        <section id="authors" className="py-24 bg-[#F5F4F0] border-t border-stone-300">
           <div className="container mx-auto px-6">
                <div className="text-center mb-16">
                    ...
                    <p className="text-stone-500 max-w-2xl mx-auto">A collaboration between MLAI CORPORATION, Google DeepMind, and Google Quantum AI.</p>
                </div>
                
                <div className="flex flex-col md:flex-row gap-8 justify-center items-center flex-wrap">
                    <AuthorCard 
                        name="Donald Dark" 
                        role="MLAI CORPORATION" 
                        delay="0s" 
                    />
                    <AuthorCard 
        ...
        <section className="py-20 bg-white border-t border-stone-100">
          <div className="container mx-auto px-6">
            <div className="flex flex-wrap justify-center items-center gap-12 md:gap-24">
              <PartnerLogo name="MLAI" subtitle="CORPORATION" />
              <PartnerLogo name="Google" subtitle="DeepMind" />
              <PartnerLogo name="Google" subtitle="Quantum AI" />
            </div>
          </div>
        </section>
      ...
      <footer className="bg-stone-900 text-stone-400 py-16">
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="text-center md:text-left">
                <div className="text-white font-serif font-bold text-2xl mb-2 flex items-center gap-2">
                   <div className="w-6 h-6 bg-white rounded-full flex items-center justify-center text-stone-900 text-sm pb-0.5">M</div>
                   AlphaQubit
                </div>
                <p className="text-sm">Visualizing "Learning high-accuracy error decoding for quantum processors"</p>
                <p className="text-xs mt-2 text-stone-500">© 2024 MLAI CORPORATION. All rights reserved.</p>
            </div>
        ...
        <div className="text-center mt-12 text-xs text-stone-600">
            Based on research published in Nature (2024). Generated by AI for MLAI CORPORATION.
        </div>
      </footer>
    </div>
  );
